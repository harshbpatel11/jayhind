...
| Which part of the business did this figure belong to? | `cost_centres`, allocated per `journal_lines.id` — P7 of TALLY-PARITY-PLAN.md §3.7. ⚠️ **An allocation is never an extra journal line** (§2.4): the entry balances without it, so every figure on every statement is identical whether the dimension is used or not. That is P7's gate, and it is why nothing in `cost-allocation.const.ts` knows what a debit is |
