...
- **Report drill-down is one spine, not a link per report** (P3d, 2026-08-29).
  A row emits a **`DrillTarget`** — `group` | `ledger` | `voucher` — and
  `src/utils/drill-target.ts` is the one resolver that turns it into a URL, so a
  new report gets drill-down by emitting the right target. Three rules ride on
  it:
