...
| Why can't I tick a party's opening balance on the reference grid? | `bill-reference.util.ts` `billIsSelectable` — the wire is `against: [{ trxId }]` and `trx_payment_receipt_trxs.trxId` is `NOT NULL` behind a foreign key, so a bill the register raised on its own has nowhere to go (**P5c‑3**). It is still OFFERED, with the reason on the row: hiding it is what made 53 parties look unsettleable, and the money reaches the ledger either way as `advance` or `on-account`. ⚠️ The sentence is on the row rather than in a tooltip — a disabled control renders none |
