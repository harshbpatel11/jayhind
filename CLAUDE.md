...
| Where does a person SPLIT a figure on a voucher? | `client-front` `components/shared/voucher-cost-allocation/` — the sub-screen (`cost-allocation-dialog`) and the trigger that says whether a row is allocated (`cost-allocation-button`), P7c‑3a. Tally's own sub-screen at Tally's own moment: an amount has been typed against a head, and this asks which parts of the business it belongs to. ⚠️ The badge is why folding it behind a button is allowed at all (§9's option-bar ruling) — the grid answers *"has this been allocated?"* with nothing open |
