...
> at all.
>
> `src/const/ledger-presentation.const.ts` — the transitional rule that maps a
> ledger back to the legacy head it is presented under — now has **three
> callers left**: `party-statement.service.ts`, `trx.service.ts`'s two
> Outstanding reads and `financial-dashboard.service.ts`. P3c moves those and
> deletes the rule.
>
> ⚠️ **`ReportsService.legacyTrialBalance` / `.legacyBalanceSheet` /
> `.legacyProfitAndLoss` are the flat statements, kept and UNROUTED for one
> release.** They are the parity gate's anchor: `qa-coa-parity` captures *them*
> either side of a change, so the diff stays about figures rather than about a
> report's shape, and `qa-p3b-statements` ties the tree to them head by head.
> Do not build anything new on them; they retire with the presentation rule.
>
...
> were posted to. That is the single declared exception to the parity gate, and
> it is a **list** — `npm run qa:coa-parity -- exceptions`, 513 entries, each
> naming the party or the head it is about. Both columns are written from the
...
| Chart-of-accounts parity | `client-back/scripts/qa-coa-parity.ts` + `src/const/parity-snapshot.const.ts` — the gate for the Tally migration (TALLY-PARITY-PLAN.md §4.2): every report for every company, captured before and after, diffed **per figure**. ⚠️ Since P3b it captures the **`legacy*`** statements, not the routed ones: a snapshot of a new shape either side of the change that introduced it is a diff of a report against itself under a different name. `--mask-labels` records the Day Book's line label and the books' `particulars` as a marker for one capture pair — every figure beside them is still compared, and whether the label is right is `qa:p3b-statements` (13)–(15); a masked snapshot and an unmasked one are refused rather than diffed. `exceptions` writes the one declared exception as a **list** (513 entries naming a party or a head, derived from the repointing itself, not from `party_ledger_plan`) — feed it back with `--allow`. Deliberately **not an oracle** — it never says what a Trial Balance ought to contain, only whether it answered the same thing twice; `qa-artifacts/tests/reports/` owns the restatement. ⚠️ Its diff reports rows **added and removed**, not only changed, because `trialBalance` suppresses zero rows — so a dual-role party netting to exactly zero leaves the report with no figure moving. And an exception is a **list**: an allowance matching nothing fails as loudly as a difference nobody allowed | `npm run qa:coa-parity -- selfcheck` |
...
| Which reports do NOT go through the presentation rule? | since P3b, **all the figure-bearing ones**: the two P3a added (`ledgerReport`/`ledgerVouchers`, `groupSummary`) and the three statements (`trialBalance`, `balanceSheet`, `profitAndLoss`), all reading `acc_ledgers`/`acc_groups` directly. Three callers are left and P3c moves them — `party-statement.service.ts`, `trx.service.ts`'s two Outstanding reads, `financial-dashboard.service.ts` |
| Where is the Trial Balance's *old* shape, and why is it still here? | `ReportsService.legacyTrialBalance` / `.legacyBalanceSheet` / `.legacyProfitAndLoss` — the flat `trx_groups` reports, kept for one release as the parity gate's anchor (P3b) and reachable only through **`?view=legacy`** on their own routes. `qa-coa-parity` captures them, `qa-p3b-statements` ties the tree to them head by head, and `qa-artifacts/tests/reports/` (10,280 lines whose oracle is Σ by presentation head) reads them through that parameter. All of it goes with the presentation rule in P3c. **Don't build a screen on it** |
...
| Which legacy head does a report print this ledger's figures under? | `src/const/ledger-presentation.const.ts` `presentationGroupId` — a `legacyTrxGroupId` **is** that head; a party ledger reports under the control head of the side D3 parented it to; anything else answers **`null`**, deliberately, so an unplaceable ledger fails a census rather than landing its money in a row nobody posted to. `presentationJoinSql`/`presentationGroupExpr` are the same rule in SQL, written once for the statements that paste it. ⚠️ Transitional, and nearly gone: after P3b its callers are the three **unrouted** `legacy*` statements (the parity anchor), `party-statement.service.ts`, `trx.service.ts`'s two Outstanding reads and `financial-dashboard.service.ts`. P3c moves the last three and deletes it |
...
| Which figures did the chart-of-accounts migration actually move? | exactly one thing — a party who both buys and sells has one ledger under one control head, so Sundry Debtors falls by ₹2,51,44,323.21 and Sundry Creditors rises by exactly that. `npm run qa:coa-parity -- exceptions` writes the list; `--allow` it into the diff |
| How do I "open the ledger"? | `GET /reports/ledger/:ledgerId` → `ReportsService.ledgerReport` (monthly summary), `…/vouchers` → `.ledgerVouchers` (the drill). **`:ledgerId` is an `acc_ledgers` id** — `group-summary/:groupId` takes an `acc_groups` one and `group-statement/:groupId` a legacy `trx_groups` one, three id spaces on one controller for as long as the two charts coexist. Passing the wrong one answers a 404, which is right and confusing, so the parameter names say which |
...
| Where does a ledger the DATA IMPORT creates land? | `src/const/import/import-group-tree.const.ts` (the plan + `sourcePlacementApplies`) → `ImportCommitService.commitGroupTree` / `.placeLedgerForHead` → `ledger-resolution.ts` `provisionLedgerForHead` (P2b‑3c). Where the **source** filed it, for a plain account head only. ⚠️ A **party** ledger and an **instrument** ledger are deliberately not placed from the source: the party branch of `presentationGroupId` matches the two control groups *exactly* (a party under a sub-group has no presentation head and its money leaves every statement), and an instrument's group comes from its `AccountType` and is a **system** ledger `describeLedgerMoveBlock` refuses to move. Both wait for P3 |
