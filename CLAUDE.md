...
- **The repair is `PostingService.rebuildBalances`**, reachable through
  ...
  **no caller at all** until 2026-08-24, so a drifted cache could not be fixed
  ...
  nothing will tell you. **And since P5b/P7b two satellites ride the same
  seam** — `bill_references` (the bill register) and `cost_allocations` (the cost
  dimension) — for the identical reason: each is a statement *about journal
  lines*, so the one place that writes them is the only place that can hold it. ⚠️ **A cache and its rebuild are one change, not two** —
...
- **The nav is two columns** (`client-front` only): `app-nav-rail` lists every
  ...
  - **Transaction ▸ Chart of Accounts** (2026-08-29 as *Ledgers*; renamed
    ...
    **Masters ▸ Transaction Group deliberately stays**: `trx_groups` is still the
    ...
    `groupFor`.
  - **Transaction ▸ Masters ▸ Cost Centres** (P7c‑2, 2026-08-31) is the cost
    dimension's three masters on one screen: the categories beside the selected
    category's centre **tree**, and the percentage **classes** on a second tab.
    Filed under Masters rather than beside Transaction ▸ Ledgers because it is
    not a second chart of accounts — a cost centre posts nothing and says which
    part of the business a figure belongs to, which is where Tally files it too
    (Accounts Info ▸ Cost Centres). ⚠️ The tree is scoped to **one category at a
    time** and there is deliberately no "all categories" row, unlike the Chart of
    Accounts' *All ledgers*: the categories are parallel partitions of the same
    figure, so a combined tree would be several roots printed as one and the
    counts beside them would overlap.
  - **Transaction ▸ Masters ▸ Currencies** (P8d‑2, 2026-09-01) is the currencies
    beside the selected one's dated **quotes**. ⚠️ Its key is **`acc-ledgers`**,
    not one of its own, and it follows the API's: the base currency IS the unit
    every figure in the chart of accounts is stated in, so a role that may shape
    the chart is the role that decides this, and a separate key would be one more
    row in every company's permission matrix for a screen with no independent
    audience. The **Revaluation** report beside it carries `reports` — P8c‑2's
    split with one noun changed: *a role that may read an exposure must not be
    able to change the rate it is measured at.* ⚠️⚠️ The **base currency is
    offered no rate pane at all**, because the API refuses a rate against itself;
    that is a refusal expressed as an absence rather than as P3d‑2's
    stays-clickable, since it is not a condition anybody can correct.
  - **Transaction ▸ Masters ▸ Scenarios** (P8e‑2, 2026-09-01) is the named sets
    of postings kept out of the books, each with how many entries it holds and
    their net effect — *"what would including this do?"*, answered before a
    report is run. ⚠️ Its key is **`reports`**, not one of its own, following the
    API's: a scenario is a way of *reading* the books, so naming one moves no
    figure and what it changes is what a report offers to include. Filed under
    **Masters** all the same, because it IS master data somebody maintains and
    the Reports sub-nav is a row of statements — the same split the three
    inventory reports use in the other direction (they live under `reports/` and
    carry `product-quantity`, UI-006). ⚠️⚠️ The **picker** that applies one lives
    on the Trial Balance, Profit & Loss and Balance Sheet
    (`components/shared/scenario-picker/`) and renders **nothing** when the
    company has no active scenario: an always-present control on the three
    busiest reports in the product would be clutter on every installation and a
    question on none. That is deliberately *not* P8d‑2's Revaluation ruling — a
    **report** that can only be empty must say which ingredient is missing,
    because a blank table reads as a defect; a **toolbar control** that is absent
    reads as nothing at all. ⚠️ And applying one is **announced**
  ...
  - **Transaction ▸ Reports carries the cost dimension** (P7d‑2, 2026-08-31)
    ...
    prints a caption where every other report prints a total row**: the
    categories are parallel cuts of the same figure, the API says so by having
    no field for a total, and a reader who has read neither file has no other
    way to know they must not add the rows up.
...
- **Report drill-down is one spine, not a link per report** (P3d, 2026-08-29).
  ...
  **P7d‑2** `cost-category` | `cost-centre` — and `src/utils/drill-target.ts` is
  the one resolver that turns it into a URL, so a new report gets drill-down by
  emitting the right target. ⚠️ The last two are a **second dimension**, not two
  more nodes of the first: a cost centre answers *which part of the business*,
  never *which side of the books*, so nothing resolves a `cost-centre` onto a
  statement. Three rules ride on it:
...
| The cost dimension's masters | `client-back/scripts/qa-p7a-cost-masters.ts` — P7a's gate (§3.7). Twelve properties over every company: the seeded primary category, the centre tree's paths and its **parallel** ancestry, and then the invariant on a **real posted line of a real switched-on ledger** — (7c) *two parallel categories take 2× the line and the rule is quiet*, which a whole-line Σ fails, with (7d) computing the per-category sums **in the gate** rather than trusting the rule's arithmetic. ⚠️ **The whole feature has no instance anywhere**: `costCentresApplicable` is `0` on all 1,383 ledgers of all 14 companies, there are zero centres, and the real Tally backup carries one category, eleven switched-on ledgers and **no centres at all**. So (5)–(10) construct everything — two categories, a two-level tree, a switched-on ledger — inside a rolled-back transaction, and (0) asserts nothing survived. P5b's `advance` arm a fourth time. ⚠️⚠️ (6) is P7's own gate sentence in the only form available before allocations exist: the Trial Balance captured either side of all of it, figure by figure. Shown to fail four ways, and the third injection was caught **twice** — once by its own property and once by a refusal asserted against its sentence (*"still holds 2 cost centres"*, which counted 3) | `npm run qa:p7a-cost-masters` |
...
| Interest | `client-back/scripts/qa-p8c-interest.ts` — P8c's gate (§3.9). Twenty-eight properties, and (7) is the phase's own **prohibition** rather than a feature: the three statements captured either side of a full set of interest parameters, and `journal_entries`/`journal_lines` counted, **while ₹1,47,740.48 of interest was claimed** — §3.9's *"posting it is an explicit Debit Note the user accepts, never automatic"*. ⚠️ Every figure is worked out **in the gate**, and (5) asserts the two day-count bases give **different** answers on the same bills, because without that (3) could pass on a build ignoring the basis. ⚠️⚠️ **Two properties could not fail.** (2) compared the gate's own restatement against the gate's own literals and never touched `daysBetween` — the injection removing the 30/360 day-of-month cap **passed 27/27**; it is three-way now (literal · restatement · implementation). And the cap test used **two 31sts**, where the cap cancels: only `31 Jan → 1 Mar` tells the two apart. ⚠️ **Three arms have no instance and are constructed**: all 2,848 `against` references are dated the same day as their bill (so the `asOf` bound on settlements is unexercised by history), **0** live bill references sit on a non-live entry (so `liveEntrySql` here is belt-and-braces — injection 5 passed 27/27 with the edit verified), and the retired-bill exclusion runs in whichever company has one. **28/28**, shown to fail six ways | `npm run qa:p8c-interest` |
| The interest screens | `qa-artifacts/tests/ui/masters/interest.ui.spec.ts` + `interest-rules.ts` (restated) — P8c‑2's gate, three properties: a rate set through the dialog with its refusals made in the browser (**counted**, not assumed), the report saying **in words** that it claims rather than charges, and *Charge no interest* **removing** the row rather than zeroing it. ⚠️ Three tests, not six, deliberately — `qa:screens` shares one 100-req/min bucket and P8b‑2's first version took another file's passing test down with two SPA boots in one test. ⚠️⚠️ It found a real defect: `isCompound` was a `computed()` over `form.getRawValue()`, and a reactive form's value is **not a signal** — so it evaluated once, cached, and the compounding-interval field never appeared. **A `computed()` is only as reactive as its narrowest source.** ⚠️ And two selector traps `parity.ts` had already written down: `<mat-table>` has **no `<tbody>`** (use `rowContaining`), and `td.num` includes the right-aligned Actions cell, so `last()` reads a button's text. **3/3**, shown to fail three ways | `npm run qa:screens` |
| The currency screens | `qa-artifacts/tests/ui/masters/currencies.ui.spec.ts` + `currency-rules.ts` (restated) — P8d‑2's gate, three properties: a currency and a dated quote **built through the dialogs** (construction first, because the feature has no instance — 0 currencies, 0 rates and 0 of 45,460 annotated lines), every refusal the browser has the facts for stated with **no request leaving the page** (counted), and the one it does not — the base currency, once anything has posted — arriving from the server in the same sentence. ⚠️ That last one asserts **both halves at once**: the dialog states the consequence, **Save stays live**, the request goes, and the sentence comes back over the wire; injection 3 is the browser guessing it and fails on both. ⚠️⚠️ It found [BUG-0072](qa-artifacts/docs/bugs/BUG-0072.md) — both deletes soft-deleted into a **unique index**, so a currency code and a quoted day could never be used again, with no restore route and a message reciting a database index's name. ⚠️ And injection 1 **passed twice** because the rebuild-waiter looped on `SECONDS`, which is a float in zsh: every wait reported *"no rebuild"* instantly while the injection went untested. **A harness that cannot wait is a harness that cannot fail.** **3/3**, shown to fail six ways | `npm run qa:screens` |
| Multi-currency | `client-back/scripts/qa-p8d-currency.ts` — P8d's gate (§3.9). Twenty-six properties, and (1) is the phase's claim as a **census**: the three annotation columns are `NULL` on all **45,460** pre-existing lines and `Σ Dr = Σ Cr = ₹6,21,920,688,281.08` unchanged, which is what makes §3.9's *"unaffected by construction"* a property of the schema rather than of care. ⚠️ (3) captures the three statements with a real foreign-currency line present and absent, 44 figures. ⚠️⚠️ **Injection 6 passed and the CODE was wrong**: `describeBaseCurrencyBlock` was applied to `update` alone, so creating a *new* currency and marking it base walked past the phase's most consequential refusal — §13's shape on the two doors a person comes in through. ⚠️ Two properties could not fail: the fixture's entry was dated after every date the gate asks about, so (9) failed with `undefined` while **(7) passed vacuously** — *"the row has no rate"* and *"there is no row"* are the same `undefined` to a `!row \|\|` check. **26/26**, shown to fail seven ways | `npm run qa:p8d-currency` |
| Scenarios | `client-back/scripts/qa-p8e-scenarios.ts` — P8e's gate (§3.9). Twenty-one properties, and it is built round **both** halves of *"includable in a report without being in the books"*: (2) posts a real balanced provisional entry and holds the three statements byte-identical, (3) asks for the scenario and requires them to move by **exactly** that figure. A gate that only checked the first passes on a scenario that does nothing. ⚠️ (1) is the census — `scenarioId` is `NULL` on all **15,474** entries — which is what makes extending `liveEntrySql` provably figure-neutral. ⚠️⚠️ **Two properties failed for reasons that were the test's**: `ledgerTrialBalance` had no `tx` parameter (the only statement without one), so both calls read a world without the constructed entry and it reported that the view *"ignored the scenario"*; and (0) found a **tombstone**, because `destroy` is paranoid and a `COUNT(*)` census counts it — *a soft delete is not a cleanup when the census is `COUNT(*)`*. **21/21**, shown to fail five ways | `npm run qa:p8e-scenarios` |
| Putting a voucher IN a scenario | `client-back/scripts/qa-p8e2-scenario-write.ts` — P8e‑2's gate (§3.9), and it exists because P8e shipped the column a report reads and **no writer at all**. Forty properties, and (4) is the centre: `liveEntrySql` takes a scenario entry out of every *report*, and `persistLines` writes four things beside the lines that **no report filters** — `bill_references` (the annexure would chase a provisional invoice), `cost_allocations` (the cost reports sum grossly on purpose, P7b) and the three balance caches (`getFundsSummary` reads the cache instead of the lines, BUG-0042). A gate that only re-checked the statements would pass on the implementation this phase was written to avoid. ⚠️ (5) is the one that bites silently: a reversal that dropped the scenario puts the **negation** of a figure that was never in the books INTO them — and the three statements **cannot see it** (`liveEntrySql` drops every reversal by `isReversal = 0` first), so (5b) reads the caches, which is BUG-0044's *a gross figure is what a cancelling pair hides behind* one dimension over. ⚠️⚠️ Its first cut read each cache as **Σ over every row** and injection 1 proved that inert: a balanced entry moved a group +₹3,14,159.26 and another −₹3,14,159.26, so two of three cache properties were blind and only `trx_accounts` — where a Journal names ONE account — reported anything. Each cache is read **per row** now. ⚠️ And (8a) failed for a reason that was the test's, asserting the FIXTURE's ledger id where `resolveLegs` decides which ledger a leg lands on; it reads the entry's own lines, and `ledgerReport` gained the optional read-only `Transaction` the other four statements have — P8e's own finding, one report over. **40/40**, shown to fail six ways | `npm run qa:p8e2-scenario-write` |
| The scenario screens | `qa-artifacts/tests/ui/masters/scenarios.ui.spec.ts` + `scenario-rules.ts` (restated) — P8e‑2's gate, three properties: a scenario **built through the dialog** (construction first, because the feature has no instance — 0 scenarios anywhere) with its name refusal made in the browser and **no request leaving the page** (counted), a **Journal put in one** where a payment is offered no field at all, and a statement read with one applied **saying in words that it is not the books**. ⚠️ That last is the phase's whole safety property on the reading side: a Balance Sheet with a scenario applied still **balances**, so *"does it add up?"* says nothing about whether a reader is looking at the books. ⚠️⚠️ Three tests, not six — `qa:screens` shares one 100-req/min bucket and P8b‑2's first version took another file's passing test down with two SPA boots in one test. ⚠️ Its own sweep depends on BUG-0073 being fixed: a leftover tombstone would be a 409 on the next run's first create, on a screen working perfectly | `npm run qa:screens` |
...
| The voucher's cost allocation panel | `qa-artifacts/tests/ui/money/voucher-allocation.ui.spec.ts` + `voucher-allocation-rules.ts` (restated) — P7c‑3a's gate, seven properties: **construction first** (the feature has no instance anywhere, so a category, a tree, a class *and a switched-on ledger* are built by the file), the panel appearing on that head and **on no other row** (a Payment, Receipt and Contra have no `ledger` row at all), a class expanding **to the paisa**, a malformed row refused in the server's sentence with **no request leaving the page**, an **incomplete** split still saving, the stored rows coming back on an edit, and an N-line Journal's allocations moving when a line is removed. ⚠️ **Approval is deliberately not driven** — the rows asserted are `trx_cost_allocations`, the draft's own statement; `ApprovalService.transition` commits its own transaction, so a posted scratch voucher is a figure the parity harness would capture with nothing behind it (`qa:p7b-cost-allocations` does that arm in a rolled-back transaction). ⚠️⚠️ **Injection 2 passed first time and the FIXTURE was the problem**: ₹1,000 at 33.3333×2 + 33.3334 has every exact share below a half, so `round` and `floor` agree and the two algorithms are indistinguishable — check 13 failed on three vectors while this gate stayed green. The split is solved now (`33.3335 / 33.3335 / 33.3330`, landing exactly on a half), and it measures the tie-break too. **7/7**, shown to fail five ways | `npm run qa:money` |
| The Cost Centres masters screen | `qa-artifacts/tests/ui/masters/cost-centres.ui.spec.ts` + `cost-rules.ts` (restated) — P7c‑2's gate, and its shape differs from the Chart of Accounts' for one measured reason: **the whole feature has no instance anywhere** (0 centres, 0 classes, `costCentresApplicable = 0` on all 1,383 ledgers), so **construction is the first property rather than the last** — a category, a two-level tree and a class, all built through the dialogs, with the terminated `path` and the per-line category asserted in the database. Then the refusals: a move into the centre's own subtree, a category still holding centres, the primary category, a centre with children, a centre a class names — each with **no request leaving the page**, counted, because a 400 caught and toasted looks identical on screen. ⚠️ **The allocation arm is deliberately not driven here** and the file says so: reaching it needs a posted allocation, which is `qa:p7a-cost-masters` (10) and `qa:p7b-cost-allocations`' job — what IS measured is the half that would break if the browser started guessing, a centre deleted **by a request that actually goes**. ⚠️⚠️ Its cleanup is the API's, because `framework/db` is read-only: both stages of the two-stage delete AND the archived view, since `cost_categories` is `UNIQUE(companyId, name)` including tombstones — a leftover is a 409 on the next run's first create, on a screen that is working perfectly | `npm run qa:screens` |
...
| Cross-repo mirror drift | `scripts/check-mirrors.js` (**this** repo — only it sees both submodules). Checks 1–3 compare data; check 4 compares **behaviour**, running both `voucher-lifecycle` implementations against `scripts/vectors/` (§13.4); check 7 compares the **hub console's** names for the nine licence switches against the hub API's `COMPANY_FEATURE_COLUMN` *and* `UpdateCompanyFeaturesDto`'s declared fields (BUG-0066 — the pair that had never once agreed); check 9 compares `JobWorkBoardStage` and `BOARD_STAGE_SEQUENCE`, **membership and order**, because the sequence IS the Kanban's lane order and the strings are the tokens the server's `stage` filter compares; check 10 runs both copies of the chart of accounts' five `describe*Block` refusals over `scripts/vectors/ledger-rules.vectors.json` (182 rows from 21 region cases) and compares the **message text**; check 11 compares the voucher **entry mode** of all fourteen types as data and *runs* `accountingRowPlan` on both sides — the backend derives it from `buildLegs`, so this is the only thing tying the entry screen to the posting engine across the repo boundary — there the sentence IS the deliverable, so a pair that agrees about the verdict and not the wording is the drift worth catching; check 12 runs both copies of `planBillSettlement` — how much of each selected bill a payment or receipt closes — over `scripts/vectors/bill-settlement.vectors.json`, comparing the **mappings and the message text**, plus `billSettlementSign`'s four rows and (P5c‑3) `allocationTargetFor`, whose two answers are checked against the **fields `CreateUpdateTrxPaymentReceiptTrxDto` actually declares** — `forbidNonWhitelisted` turns a key the DTO does not declare into a 400, which is BUG-0066's shape and why check 7 is written the same way. ⚠️ check 13 runs both copies of the cost dimension's **seven** `describe*Block` refusals over `scripts/vectors/cost-rules.vectors.json` (134 rows from 49 region cases), comparing the **message text** — check 10's argument one master across, with two backend files behind one mirror and `allocationCount: null` stating *"not known here"* as a rule rather than an accident of two signatures. ⚠️⚠️ It also carries a second comparator, the only one in this file that is not about a sentence: `expandCostCentreClass` — a **stencil**, expanded in the browser, whose ROWS are what a voucher saves — is compared share by share in **integer paisa with `===`**, never `near`, because P7c‑1 measured a paisa-tolerant check reading ₹0.06 as equal to ₹0.05 and passing (P7c‑3a). ⚠️ check 14 runs both copies of the budget refusals over `scripts/vectors/budget-rules.vectors.json` (62 rows from 29 region cases), comparing the **message text** — and deliberately does **not** compare the variance verdict, because the browser has no copy of it: the report renders the server's answer, and P8b‑2's first cut mirrored it into a function nothing called, which the UI gate proved by injecting a wrong verdict there and passing 6/6. **A mirror with no reader is not a mirror.** ⚠️ check 16 runs both copies of the **four** currency refusals over `scripts/vectors/currency-rules.vectors.json` (112 rows from 15 region cases), comparing the **message text**, plus `RateType` as data — and the enum being compared **both** ways is deliberate: a fourth rate type on one side is caught by the data comparison *and* by the `bad-type` vector, which carries `mid` for exactly that. ⚠️⚠️ Two of its four arms are passed **`null`** (*"not known here"*) — whether the company has posted, and how many lines name the currency — because the browser holds neither, which is check 13's `allocationCount: null` again; the conversion arithmetic is deliberately absent for check 15's reason. That check is why the rule lives in `bill-reference.const.ts` rather than beside `planSettlement`: `settlement.const.ts` imports a Sequelize entity and cannot be bundled. ⚠️ check 17 runs both copies of the **three** scenario refusals over `scripts/vectors/scenario-rules.vectors.json` (54 rows from 12 region cases), comparing the **message text**, plus `SCENARIO_VOUCHER_KINDS` and `ScenarioKind` as data — the set being compared as data is what stops one side *offering* the scenario field on a voucher the other refuses, where the sentence alone would only disagree about wording. ⚠️⚠️ Unlike check 16, **neither arm is passed `null`**: `entryCount` is on the row `GET /scenarios` already returns and the voucher kind is the segment the entry screen is routed on, so both refusals are the browser's and never leave the page. `describeScenarioMoveBlock` has **no browser copy at all** and is deliberately not compared — nothing in the SPA edits `journal_entries.scenarioId`, and the one path that changes a posted voucher's scenario does it by *reversing*, which is what that rule prescribes rather than forbids; mirroring it would be P8b‑2's reader-less copy a second time. Needs esbuild from one submodule's `node_modules` and **fails loudly** rather than downgrading if none is present | `node scripts/check-mirrors.js` |
...
| Which part of the business did this figure belong to? | `cost_allocations`, one row per `journal_lines.id` per cost centre — P7b of TALLY-PARITY-PLAN.md §3.7, written from `PostingService.persistLines` (the one writer of journal lines, beside the bill register and for the same reason). ⚠️ **An allocation is never an extra journal line** (§2.4): the entry balances without it, so every figure on every statement is identical whether the dimension is used or not. That is P7's gate — `qa:p7b-cost-allocations` (3), asked as a deletion — and it is why nothing in `cost-allocation.const.ts` knows what a debit is |
| Where does a VOUCHER say which cost centres it belongs to? | `trx_cost_allocations`, keyed `(ownerType, ownerId)` over **D6's four `ledgerId` holders** (`trx`, `trx_charges`, `trx_payment_receipts`, `trx_payment_receipt_lines`) — a row that names no ledger has nothing to allocate, which is the rule the set comes from rather than a list. ⚠️ Two tables because a **draft has no journal line**: a voucher posts at approval and an edit reverses and supersedes it, so the statement has to survive three states in which no line exists. Exactly `trx_payment_receipt_trxs` beside `bill_references`. ⚠️⚠️ `ownerId` carries **no foreign key**, so `companyId` is the only thing keeping one tenant's owner id off another's rows — `sharesForOwners` binds it and filters the `IN × IN` cross product back down to the pairs asked for |
| Why is a cost allocation `−500` when the operator typed `500`? | the sign is the **leg's**, applied by `signAllocationsForLeg` in `persistLines` — a sales head is a credit leg. The wire carries a magnitude because *which side* is `buildLegs`' answer, not a person's, and `describeAllocationPayloadBlock` refuses a negative rather than flipping it. That one decision also covers a negative charge flipped by `resolveLegs`, and the reversal below |
| Why does a cancelled voucher's cost allocation NEGATE where its bill reference RETIRES? | because an allocation is **signed** and a reference is a magnitude. A stranded reference breaks `Σ|ref| = |line|` on a dead line, so it must be retired (P5b); a negated allocation cancels its original, so a **gross** Σ that forgot `liveEntrySql` is still right — BUG-0044's lesson applied in a new dimension before it could be repeated. `reverseAllocations`, and the rows are **copied from the original line**, never re-derived from the voucher (`ledgerId`'s own ruling) |
| Why did my cost allocation refuse when §3.7 says it only warns? | the two halves are different questions. The **books** being incomplete — a partial or missing allocation — is `costAllocationProblems`, a warning P7d reads. The **payload** being malformed — another company's centre, a centre under the wrong category, a negative share — is `describeAllocationPayloadBlock`, refused in a sentence, because a row stored in silence is summed by every cost report under a partition it does not belong to (BUG-0040's shape). Its spec asserts a ₹1 allocation of a ₹10,000 row goes straight through |
| How does an entry screen read a voucher's allocations back? | `POST /cost-allocations/list` — `@SharedRead({ parties: false })`, so a trading party is refused (D-46: what a supplier's invoice was allocated to is this company's cost structure). ⚠️ It shipped with the writer rather than with P7c's screen because `TrxWriteService` **destroys and rebuilds `trx_charges` on every save**: their ids change, so a form that could not read them back would lose a charge's cost centres on any edit, silently |
| Why is the allocation Σ taken PER CATEGORY? | because the categories are **parallel** partitions of the same figure — `cost-allocation.const.ts`, P7a. One expense allocated to two departments and two locations is four rows totalling 2× the line, which is **correct**; a whole-line Σ reads it as a 100 % over-allocation. ⚠️ It is **signed**, unlike `referenceCoverageProblem`'s Σ of magnitudes: a credit note against a department must *reduce* its spend, so an unsigned column is a report that cannot subtract |
| Which categories does this ledger have to be allocated across? | `cost-allocation.const.ts` `applicableCategories` — the ledger's own `costCentresApplicable` switch, then its nature's **statement** crossed with the category's two Tally flags. ⚠️ The only reader of those flags: asking them anywhere else gives the answer a second definition, which is BUG-0046's shape. ⚠️⚠️ `null` nature → no category applies, because `Suspense A/c` and `Branch / Divisions` have no side of the books and a rule that only warns must not invent one |
| Why is a partial cost allocation not refused? | §3.7 — *"a warning on a reconciliation report, not a refused save"*. Nothing in `cost-allocation.const.ts` throws and a test asserts it, the same ruling and the same test `statutory-windows.const.ts` carries. A line with **no** allocation at all is reported too: a rule that checked only the rows that exist could never report the commonest incompleteness there is |
| Why must a cost centre's parent be in its own category? | `describeCentrePlacementBlock` — every cost report rolls a subtree up by `path` prefix **within one category**, so a *location* under a *department* would be collected by a Department Summary with no business seeing it and the two totals would overlap. The parallel-category Σ is only true if the trees are parallel too |
| How do I allocate the same split over and over without typing it? | a **cost centre class** — `src/const/cost-centre-class.const.ts`, P7c‑1 (§2.4). `describeClassLinesBlock` is why one cannot be stored and `expandCostCentreClass` turns its percentages into rupees; they are one statement about the same numbers, which is why they share a file. ⚠️ Refused on the **master** where §3.7 has a partial allocation only *warn* on a voucher: a definition that totals 90 % can never produce a complete allocation, so accepting it warns on every voucher it is used on at a screen with no way to say why |
| Why does a voucher not record which cost centre class was used? | because it is a **stencil**. The screen expands it into the `trx_cost_allocations` rows the wire already carries and what is saved is the rows — a stored class id beside them is two statements about one split that come apart the moment somebody edits the class, so a voucher entered last March would be *reported* as one split and *re-expand* as another. `invoiceBodyOf`'s ruling one phase across. ⚠️ Nothing points at a class, which is the whole of `describeClassDeleteBlock`: it returns `null` unconditionally, and exists to say so rather than to be called |
| Why is 1/3 of ₹1,000 not `333.33 · 333.33 · 333.33`? | `expandCostCentreClass` is the **largest-remainder method in integer paisa** — floor each line, then hand the leftover paisa out largest-fractional-remainder first. A lost paisa is a `costAllocationProblems` coverage warning on **every voucher the class is ever used on**. ⚠️ Round-then-correct-the-largest is the answer that reads as equivalent and takes a share **negative** on ten lines over five paisa, which `describeAllocationPayloadBlock` refuses — a class that made its own voucher unsavable. ⚠️⚠️ A share that rounds to nothing is **dropped**, for the same reason, and the denominator is 100 rather than the lines' own sum, so a class that somehow got stored broken under-allocates visibly |
| Where does a person SPLIT a figure on a voucher? | `client-front` `components/shared/voucher-cost-allocation/` — the sub-screen (`cost-allocation-dialog`) and the trigger that says whether a row is allocated (`cost-allocation-button`). **Five hosts**: a Journal's header head and each of its N lines (P7c‑3a), and on the item form the voucher's own head, the Accounting Invoice body's rows and the Charges chip's rows (P7c‑3b). Tally's own sub-screen at Tally's own moment: an amount has been typed against a head, and this asks which parts of the business it belongs to. ⚠️ The badge is why folding it behind a button is allowed at all (§9's option-bar ruling) — the grid answers *"has this been allocated?"* with nothing open |
| Why does an item LINE have no allocation button? | a `trx_items` row names a **product**, never a ledger, so it has nothing to allocate — Tally allocates the ledger entry for the same reason, and `CreateUpdateTrxDto.costAllocations`' own comment says so. A per-item split would be a different feature with its own owner type. ⚠️ On the item form the head's figure is the **item net** (`trx.totalAmount`), which is what the goods leg carries — so on an **Accounting Invoice** it is zero, the head offers nothing, and the money is allocated on the `trx_charges` rows that hold it. That falls out of the figure rather than from a rule anybody wrote (P7c‑3b) |
| Where do a voucher's allocations live on the item form — a signal or the form? | **the form** — a control on the root for the head, one inside each charge's own `FormGroup` (P7c‑3b), where the Dr/Cr surface keeps signals beside it. Deliberately: this screen sends `transactionForm.value` verbatim so the payload needs no surgery, and **a removed charge takes its split with it structurally** — the hand re-keying `dropLineKey` exists for cannot arise, and only the map of head *options* still needs it. ⚠️ An array is always sent, empty when nothing is allocated: `TrxService.saveCostAllocations` reads *absent* as "not talking about allocations" and `[]` as "clear them", and this screen states the whole picture |
| Why did switching invoice body lose my cost allocation? | because `toggleInvoiceBody` **discards** — the two bodies are different tables and there is no honest mapping between an item line and a ledger allocation (P4e). It confirms first. ⚠️ P7c‑3b's gate was written the other way round, expecting the split to follow the row into the Charges chip, and failed: the chip is a **third host**, not the same row seen twice |
| Why is there no allocation button on this voucher row? | either the head's `costCategoryIds` is empty — which is the ordinary case, `costCentresApplicable` being `0` on all 1,383 ledgers of all 14 companies — or the row is **not a head a person picked**. On the Dr/Cr grid only a **Journal** has a `ledger` row (`accountingRowPlan`), because `postPaymentReceipt` reads `trxGroupId` only for a Journal, so a Payment's or Receipt's head is a classification and not a leg. ⚠️ That is not a condition written on the screen; it is where the trigger is rendered. Rows stored against a non-leg head would be read by **nothing** — the operator would have allocated a figure no report can show |
| Which cost categories apply to this voucher head? | `VoucherHeadOption.costCategoryIds` — the **server's** answer, from `applicableCategories`, put on the picker option (P7c‑3a). ⚠️ The answer and not the inputs, deliberately: sending `costCentresApplicable` + the nature would make the screen re-derive the kind, and `costItemKindOf` is a *derivation* from each nature's own financial statement (D-54's shape) whose mirror could only be a list. ⚠️⚠️ **"Revenue" is Tally's word for a P&L item, not for income** — an **Expense** head is a revenue item, so a revenue-only category applies to it. Reading it as "income" makes an Expense head answer no categories at all; this phase's own unit spec made that mistake first |
| Why did my voucher's allocation vanish when I changed the head? | only the rows whose **category no longer applies** are dropped (`keepApplicable`, P7c‑3a). Carrying them all would store a figure `costAllocationProblems` reports `NotApplicable` for ever; clearing them all would punish re-picking a head inside the same kind, which is an ordinary correction |
| Why is a cost allocation refused on the voucher when §3.7 says it only warns? | two different questions, drawn differently on purpose. The **payload** being malformed — a centre in the wrong category, a negative or zero share — is `describeAllocationPayloadBlock`, refused with Accept disabled. The **books** being incomplete is a *coverage hint per category* and Accept stays live: a gate there would refuse the ordinary case (part of a figure, come back to it) and the **default** case (a company that has just switched a ledger on has every voucher unallocated). `check-mirrors` check 13 carries `alloc-empty` and `alloc-partial` so a later "improvement" cannot turn the panel into a gate |
| Why did removing a journal line move its cost allocation? | it should move — `voucher-entry.ts` `dropLineKey` (P7c‑3a). The maps are keyed by **position**, so without re-keying, removing line 1 hands line 2's split to line 1 silently, against a different head. Gate property (7) is what measures it |
| Does the voucher remember which cost centre class was applied? | **no**, and that is P7c‑1's central ruling — a class is a stencil. `expandCostCentreClass` runs in the **browser**, the operator sees the rows and may edit them, and what is saved is the rows. `check-mirrors` check 13 therefore compares the expansion **share by share in integer paisa with `===`**, never `near`: P7c‑1 measured a paisa-tolerant check reading ₹0.06 as equal to ₹0.05 and passing, and a stencil the two sides expand differently means the operator reads one split while the books carry another |
| Where does a person SET a budget, and read it? | `client-front` `components/admin/transaction/budgets/` (`/transaction/masters/budgets`, key `budgets`) and `…/reports/budget-variance/` (key `reports`) — P8b‑2. Two keys on purpose: a role that may read the statements may read how they compare with the plan, and editing the plan is a separate decision. Gate `npm run qa:screens` (`budgets.ui.spec.ts`) |
| Why is the variance verdict not mirrored in the browser? | because the report **renders** it — it is formed once in `budget.const.ts` and arrives on the row. P8b‑2 mirrored it anyway, nothing called the copy, and the UI gate's injection aimed at it **passed 6/6**. ⚠️ **A mirror with no reader is not a mirror** (§6.4's rule one layer up); a mirror exists so a screen does not *offer* what the server will refuse, and a verdict is not an offer. `check-mirrors.js` check 14 carries the whole argument |
| Where does a person SET an interest rate, and read what it claims? | the ledger's own row on `/transaction/ledgers` (dialog, key **`acc-ledgers`**) and `/transaction/reports/interest` (key **`reports`**) — P8c‑2. ⚠️ Two keys on purpose: a role that may read what is owed must not be able to quietly change what is charged |
| Why is the interest CALCULATOR not mirrored to the browser? | because the report **renders** figures the server computed and the browser never recomputes one, so a copy would be a rule with no reader — which P8b‑2 shipped and its own gate proved by injecting a wrong verdict into it and passing 6/6. Only `describeInterestTermsBlock` is mirrored, because the dialog uses it to refuse a rate before the request leaves the page. `check-mirrors.js` check 15 carries the argument |
| Why does the interest screen say "never charged" instead of just having no Post button? | because the absence of a button is not a statement. §3.9's *"an explicit Debit Note the user accepts, never automatic"* is a design decision, and *Raise a debit note* **navigates to a blank one** rather than creating it |
| How do I model a figure without putting it in the books? | a **scenario** — `journal_entries.scenarioId`, P8e (§3.9). `NULL` is a real entry; anything else is excluded by `liveEntrySql` **for all 57 of its callers**, so a provisional figure is out of every report without any of them being told. A report given `?scenarioId=` sees the books **plus** that scenario. ⚠️ It is never a second set of books: a scenario has no entries of its own, so it cannot drift from what it annotates |
| Why is the "optional" flag on the VOUCHER and not the voucher type, when §3.9 says the type? | because Tally's voucher types are **user-definable** and this app's are a fixed enum of fourteen system kinds — so *"this type is optional"* would mean *"every sale is provisional"*. Tally's own `Ctrl+L` marks a voucher; the type-level flag exists there because Tally has no per-voucher scenario link, and this schema does |
| Why can't a posted entry be moved into a scenario? | `scenario.const.ts` `describeScenarioMoveBlock` — it would restate the accounts by editing one column, with no voucher, no reversal and nothing in the audit trail saying a figure moved. Cancel the voucher and enter it again the way it should be counted |
| Why does deleting a scenario that holds entries archive rather than erase? | not because the rows are evidence — because erasing it would **change what the surviving rows mean**. An entry whose `scenarioId` pointed at nothing would be excluded by the default (it carries a scenario) and included by no scenario (there is none): invisible to every report *and* in none of them |
| How do I actually PUT a voucher in a scenario? | `trx_payment_receipts.scenarioId` / `trx_contras.scenarioId` (P8e‑2), stamped onto the journal entry by `PostingService.post`. ⚠️ P8e shipped the column a report reads and **no writer at all** — one `grep` found `liveEntrySql` reading it and nothing writing it — so a scenario could be named and no voucher could ever be put in one. On screen it is a field on the Dr/Cr entry surface, and only where `SCENARIO_VOUCHER_KINDS` allows it |
| Why is the scenario field missing on my Sales invoice? | `scenario.const.ts` `SCENARIO_VOUCHER_KINDS` — **a Journal and a Contra only**, and `describeScenarioVoucherBlock` is the refusal. Those are the two kinds whose *entire* effect at approval is journal lines; every other kind moves **stock** (`InventoryService.apply` reads no journal entry), is a **GST document** (`GstReturnAssemblyService` assembles GSTR-1 from `trx` rows, never from `journal_lines`) or **settles a real bill** (`applyReceiptSettlement` moves `trx.paidAmount`). A scenario takes an entry out of the general ledger and out of nothing else, so a provisional invoice would be filed with the government while being out of the accounts. Widening it means teaching those three about scenarios, not editing the list |
| What does `persistLines` NOT do for a provisional entry? | everything except the lines — one early return (P8e‑2). `bill_references`, `cost_allocations` and all **three** balance caches are skipped, because none of their readers consults `liveEntrySql`: the annexure would chase a provisional invoice, the cost reports sum **grossly** on purpose (P7b), and `getFundsSummary` reads the cache instead of the lines (BUG-0042). Written as ONE return rather than four conditions, so a fifth satellite is out of a scenario by existing |
| Why does `rebuildBalances` join `journal_entries`? | to exclude a scenario's lines (P8e‑2). **A cache and its rebuild are one change** (§4.9) — without it, running the repair door with a provisional entry present silently moves provisional money into the funds card, and nothing says so because every ledger-derived report goes on excluding it. Reversals are still included: the pair cancels, which is what makes Σ over every line the right figure |
| Why does a cancelled provisional voucher not move the books? | `reverseSource` copies `live.scenarioId` onto the reversal — the same argument as `ledgerId` and `date` beside it: **a reversal has to cancel where the original landed**. Into `NULL` it would put the *negation* of a figure that was never in the books INTO them. ⚠️ The three statements cannot see that defect (`liveEntrySql` drops every reversal by `isReversal = 0` first); what sees it is the **caches**, which is `qa:p8e2-scenario-write` (5b) |
| Where does a person read a statement WITH a scenario? | the picker on the Trial Balance, Profit & Loss and Balance Sheet (`components/shared/scenario-picker/`, P8e‑2), carried in the URL beside the period and through the drill. ⚠️ It renders **nothing** when the company has no active scenario — an always-present control on the three busiest reports would be clutter on every installation; the masters screen is where the feature announces itself. ⚠️⚠️ And applying one is **announced**: a Balance Sheet with a scenario applied still *balances*, so *"does it add up?"* says nothing about whether a reader is looking at the books — `.rpt-scenario-banner` is the only thing that does |
| Why does `group-summary` take a `scenarioId`? | because it is the **expand** of the three statements, not a report anybody reaches cold (P8e‑2). Without it, expanding a group on a scenario'd Trial Balance answers the books and the children do not add up to the parent that was clicked — `qa:p3b-statements` (6)'s *"a drill-down never changes a figure"* broken by a query right about everything except which entries it may see. The Ledger report, its vouchers and the Group Book take it for the same reason |
| Why did deleting a scenario make its name unusable for ever? | it no longer does — [BUG-0073](qa-artifacts/docs/bugs/BUG-0073.md), which is **BUG-0072's shape a third time**: `UNIQUE(companyId, name)` counts a tombstone and Sequelize's paranoid read does not. Hard delete plus tombstone revival on create, exactly as `currencies` was fixed. ⚠️ P8e shipped **after** BUG-0072 was found and **before** it was fixed, so the fix and the next instance passed each other — when a defect turns out to be a *shape*, ask not only *where else is this shape?* but *what landed between the finding and the fix?* |
| Why did editing an N-line Journal reset its date and narration? | it no longer does — `patchPaymentReceipt`'s split branch patched **no header at all**, pushing the lines and returning, so the form opened on today's date with an empty narration and `onSubmit` wrote both back. Found while adding `scenarioId` at P8e‑2, which made it data-loss shaped (a provisional journal would have been saved into the books) rather than merely wrong. ⚠️ It is not `patchValue(record)` wholesale: `amount` and `trxAccountId` are deliberately absent on a split journal |
| How do I record a foreign-currency figure? | `journal_lines.currencyId · fcAmount · rate` — P8d (§3.9). ⚠️ **All three are nullable and absent means BASE**, and the 45,460 lines that predate P8d were not backfilled: `debit`/`credit` stay in base currency and stay authoritative, so every figure-bearing report is unaffected **by construction** rather than by care. A currency is an **annotation**, not a unit of account being changed |
| Where is the base currency, and why can it not be changed? | `currencies.isBase` — a row with a flag, as Tally has it, so there is one definition of what a currency is. ⚠️ `describeBaseCurrencyBlock` refuses to move it once anything has posted: every figure in `journal_lines` is denominated in it **implicitly** (there is no column saying so — which is why `currencyId` can be nullable), so re-pointing it would restate the whole ledger, every statement and every filed return. ⚠️⚠️ The rule is on **`create` and `update` both**; it was on `update` alone at first, and creating a new currency as base was the same restatement by the other door |
| Where does a person SET a currency and quote a rate, and read the exposure? | `/transaction/masters/currencies` (the currencies beside the selected one's dated quotes, key **`acc-ledgers`**) and `/transaction/reports/revaluation` (key **`reports`**) — P8d‑2. ⚠️ Two keys, P8c‑2's split with one noun changed: a role that may read an exposure must not be able to change the rate it is measured at. The masters key is `acc-ledgers` rather than one of its own because the base currency IS the unit the chart of accounts is denominated in |
| Why is the Revaluation Report empty on every company? | because **nothing writes `journal_lines.currencyId`** — P8d added the three annotation columns and no voucher states a currency (0 annotated of 45,460, measured). The empty state names which of the three ingredients is missing (no base currency · no quote · no annotated line), because *"no rows"* is the same picture whichever it is. A voucher-level currency field is a phase that has not been built, recorded in P8d‑2's record rather than implied |
| Why did deleting a currency make its code unusable for ever? | it no longer does — [BUG-0072](qa-artifacts/docs/bugs/BUG-0072.md). `currencies` is `UNIQUE(companyId, code)` and MySQL counts a **tombstone** in a unique index while Sequelize's paranoid read does not, so a soft delete left a row nothing could list, alter, restore or delete again. Same shape on `exchange_rates`, where withdrawing a wrong quote made that `(currency, type, day)` unquotable. ⚠️ Both are **hard** deletes now, which is not a relaxation of §4.9 rule 2 — `describeCurrencyDeleteBlock` refuses a currency any posted line names, so the delete is only ever reached when there is nothing to preserve. **Before making a table paranoid under a unique index, provide one of three things: a restore route, a `paranoid: false` read at the write seam, or that argument** |
| Why is the currency CONVERSION not mirrored to the browser? | check 15's argument one master over: the Revaluation Report renders figures the server computed and the browser never recomputes one, so `rateOn`, `toBase` and `unrealisedVerdict` would be rules with **no reader** (P8b‑2 shipped exactly that). Only the four `describe*Block` refusals are mirrored, in `client-front/src/utils/currency-rules.util.ts`; `check-mirrors.js` **check 16** compares the message text over 112 rows. ⚠️ Two of the four are passed **`null`** — *"not known here"* — because the browser holds neither figure they turn on |
| Which rate applies on a date? | `currency.const.ts` `rateOn` — the **latest quote on or before** it. A quote stands until the next one, so there is deliberately no `effectiveTo` (an end date would be a second statement about the same fact). ⚠️ It does **not** extrapolate backwards: a date before the first quote has *no rate*, because valuing a 2019 invoice at a 2026 rate is a figure nobody quoted presented as a fact — and the revaluation report **counts** those rows rather than showing them as no movement |
| Why does the revaluation not recompute the posted base? | because `fcAmount × rate` answers a hair differently wherever the original rounding fell, and that hair would be reported as an exchange movement that never happened — BUG-0069's doctrine, *where a figure has been posted, read the posting*. `unrealisedOn` reads `postedBase`; the gate's injection 3 measures it |
| What interest is this customer running up? | `src/const/interest.const.ts` (the calculator) · `interest-report.service.ts` (`GET /reports/interest`) · `interest_terms` per ledger (`GET\|PUT\|DELETE /acc-ledgers/:id/interest`) — P8c, §3.9. ⚠️ **Nothing posts, and nothing decides to post**: interest on an overdue receivable is a *claim*, and the Debit Note is `POST /trx` raised by a person who read the figure. No cron, no queue job, no `PostingService` path reaches this table. Gate `npm run qa:p8c-interest` (7) |
| Which day-count convention does interest use? | the ledger's own `basis` — `actual-365` or `days-30-360` (30E/360, each day-of-month capped at 30). ⚠️ ₹1,00,000 at 18 % for a quarter is **₹4,438.36** on one and **₹4,500.00** on the other; neither is rounder, and a customer disputing a note argues from whichever their contract names. `daysBetween` is the only place either is implemented |
| Why does interest read `bill_references` and not the running balance? | §3.9 says *"and over the running balance otherwise"*, and **"otherwise" is empty on this schema**: since P5b every party line carries a bill reference, so a second path would be a derivation with no population that disagrees the first time one of them is wrong. The register is a **partition** of the party's journal lines (§3.6), so the figure charged reconciles by construction |
| Why does every interest line say "bill-date"? | because **no due date exists** — measured NULL on all 11,350 `bill_references` rows and all 11,432 `trx` rows — so the effective date falls back to the bill's own posting date. The payload reports how many lines did that, deliberately: a report that used it silently would look like it was applying credit terms somebody had set |
| Why are interest parameters their own table and not columns on `acc_ledgers`? | every figure-bearing report resolves through `acc_ledgers` and P8c moves no figure; and **absence is the meaningful state** — a ledger with no row charges no interest, which is a state rather than a rate of zero, and it is what the negative-rate refusal tells the operator. `ON DELETE CASCADE`, the opposite of `budget_lines`' RESTRICT, because the parameters leave no figure behind to misread |
| What did we PLAN to spend or earn, and how are we doing against it? | `src/const/budget.const.ts` + `src/services/budget-report.service.ts` (P8b‑1, §3.9). `GET /reports/budget-variance/:budgetId` under the `reports` key; the plan itself is `/budgets` under its own `budgets` key. ⚠️ **A budget posts nothing and nothing points at one** — §3.9's *"no engine change at all"* — so every statement answers the same money whether a budget exists or not, and `describeBudgetDeleteBlock` returns `null` unconditionally. Gate `npm run qa:p8b-budgets` |
| Is this budget variance good or bad? | `budget.const.ts` `varianceDirection` / `varianceVerdict` — and for **Asset and Liability it deliberately answers `null`**: a cash target wants to be exceeded and a debtors target does not, and both are Assets; a borrowing limit wants to be undershot and share capital overshot, and both are Liabilities. Only Expense (lower is favourable) and Income (higher is) are derivable. ⚠️ Derived from the nature, never stored on the line, so a head re-parented across natures leaves no stale opinion (D-54's shape). A **nil** variance has no verdict either — calling it favourable makes every head budgeted to zero and left alone a success story |
| Why is budget variance a REPORT and not a column on the Trial Balance? | §3.9 says *"column"*, and two measurements overruled it (`BudgetReportService`'s header). A budget's lines are **sparse** — fifty heads out of 1,383 ledgers, so a column is blank almost everywhere — and a line may target a **cost centre**, which appears on no statement at all, so a column could carry two of the three target kinds and would silently drop the third. Tally shows it as its own view for the same reason |
...
| Where does §3.7's "warning on a reconciliation report" actually get read? | `GET /reports/cost-allocation-reconciliation` → `CostReportService.costAllocationReconciliation` (P7d‑1). `costAllocationProblems` had **no reader in the product** from P7a to here. It **calls** the rule rather than restating it, so this report and the entry screen's coverage hint are one sentence about one line. ⚠️ Its population is a **union** — a line whose ledger is switched **on**, *or* a line that has an allocation — because the first half finds a switched-on ledger with no allocation at all (which has no rows to be found by) and the second finds allocations stranded on a ledger somebody switched **off**, which are summed by every cost report and reconcile against nothing. ⚠️⚠️ It **refuses a period naming the limit** rather than truncating: a reconciliation that quietly stops looking is the failure mode this dimension already has too much of |
| Why do two cost gates fail on `(0) no scratch row survived`? | not their fault — the **P7c‑3 browser gate leaks**. `voucher-allocation.ui.spec.ts` creates through the real API (as it must) and leaves its `QA·P7c3 …` categories, centres and `trx_cost_allocations` rows behind, and `qa:p7b`/`qa:p7c`'s (0) is a global `COUNT(*) = 0`, so somebody else's leak reads as their own failure. `qa:p7d`'s (0) is **census-relative** — it captures the counts before the run and compares after — which is the shape the other two want. **A permanently-red gate is one nobody reads** |
| Where does a person define cost centres on SCREEN? | `client-front` `components/admin/transaction/cost-centres/` — `/transaction/masters/cost-centres` (P7c‑2), and its four dialogs: `category-add-edit`, `centre-add-edit`, `centre-move-dialog`, `class-add-edit`. Two tabs: the categories beside the selected category's centre tree, and the percentage classes. ⚠️ The tree shows **one category at a time** and there is no *"all categories"* row — the categories are parallel partitions of the same figure, so a combined tree would be several roots printed as one |
| Which cost refusals does the SCREEN make, and which does the server? | `client-front/src/utils/cost-rules.util.ts` — five of the six are the browser's, because the facts are on payloads it already holds (a centre's category and `path` from the tree, its `childCount` on the node, the number of classes naming it counted off `GET /cost-centre-classes/list`, which returns every class with its split). ⚠️ **`allocationCount` is passed `null`** — *"not known here"* — so the delete request goes and the server refuses in the same words: P3d‑2's `hasPostings` ruling, and the vector table states that equivalence as a rule rather than leaving it an accident of two signatures. `check-mirrors.js` check 13 compares the **wording** |
| Why does the class editor refuse while I am still typing? | because a split is incomplete for most of the time it is being edited, so a refusal that only arrived on Save would fire on every keystroke nobody has typed yet. `describeClassLinesBlock` runs on every change and the sentence sits under the rows it is about (P7c‑2). ⚠️ A line whose two pickers are still empty is **not** put to the rule — an unfinished row is not a wrong one, and *"Cost centre #0 is not one of this company's"* about a row somebody is halfway through is the dialog shouting at its own operator |
| Why is a master dialog's refusal styled by `.master-dialog__*` and not `.coa-dialog__*`? | it was renamed at P7c‑2. Those rules are about **a master dialog that states a refusal**, not about the chart of accounts, and the Cost Centres screen is the second screen to use them — a second copy under a second prefix is the duplication `styles/custom/_master-dialog.scss`' own header warns about. Global, for §14's `.jwd__full` reason |
| Why can't I delete this cost centre — it has no allocations? | a **class** still names it. `describeCentreDeleteBlock` asks three questions in the order somebody can act in — children, then classes, then allocations — and the class one is **refused** where the allocation one **archives**, because a class is fixable: take the centre out of it, which is one edit. The count is DISTINCT classes and joins on the class's own `deletedAt`, so an archived class holds nothing hostage |
| Why did a refusal leave a half-written row behind? | because a `throw` inside a **caller's** transaction is not a rollback — there is no savepoint. `CostCentreClassService` judges the whole split **before** inserting anything for exactly that reason (P7c‑1, found by its own gate: the header row survived the refusal, so the next five refusal properties were all answered *"already exists"*). Invisible through the controller, where the service opens its own transaction — which is what makes it the shape that survives review |
| Why is a cost centre move allowed on a centre that has posted? | because a centre's `path` carries no denormalised copy of anything, unlike `acc_groups.nature` — so re-filing it re-signs no figure, and re-organising departments between years is what a business does. `describeCentreMoveBlock` refuses only a cross-category parent and a cycle. Erasing an **allocated-to** centre is refused separately (`describeCentreDeleteBlock`): it archives, §4.9 rule 2 one dimension across. ⚠️ Since P7b the count spans **both** tables — a centre named only by an unapproved draft has moved no figure and is still held, and erasing it turns that draft's eventual approval into a foreign-key error weeks later |
...
| Which side does a report print this balance on, and what does it call the voucher? | `src/const/ledger-report.const.ts` `presentFigures` and `voucherNoFor` — **one definition each** since P7d‑1, when the cost reports became their fourth caller. Both were local to `ReportsService` (`asRow`, and a private `voucherNo`), which now delegates. `journal_entries` names a document by a `sourceType`/`sourceId` pair rather than a foreign key (§4.9 rule 2), so `voucherNoFor` **is** the number and every report that prints one has to agree about it |
