...
>
> **Every figure-bearing report now reads `ledgerId`** (P2b‑2, 2026-08-28) and
> — except the two P3a added, which read the new chart with no mapping at all —
> maps each ledger back to the legacy head it is presented under —
> `src/const/ledger-presentation.const.ts`, the transitional rule that lets the
> mechanism move without the presentation moving with it. **`trxGroupId`
> survives as a shadow** for two *label* reads (the Day Book's line label and
> the cash book's `particulars`, both of which become the **ledger's** name in
> P3), for the reads whose heads are 1:1 with a D2 ledger, and for
> `trx_groups.currentBalance`, which is its own cache. D9 drops it, a separate
> release.
>
...
| What does a journal line NAME? | **both**, since D5 (2026-08-28): `ledgerId` against `acc_ledgers` (the postable **leaf**) and `trxGroupId` beside it as a shadow. ⚠️ Since P2b‑2 **every figure-bearing report reads `ledgerId`**; the shadow survives for two label reads, the heads that are 1:1 with a D2 ledger, and its own cache. D9 drops it — a separate release. A group is a container whose balance is the sum of its children; `trx_groups` is both at once, which is why the Trial Balance prints one "Customer Dues" row where a Tally user expects a group expanding to every customer |
| Which reports do NOT go through the presentation rule? | the two P3a added — `ledgerReport`/`ledgerVouchers` and `groupSummary` — which read `acc_ledgers`/`acc_groups` directly. That is what makes them a **second derivation** of the same figures, and `qa:p3-ledger-report` (6) is the two meeting, head by head |
| Which legacy head does a report print this ledger's figures under? | `src/const/ledger-presentation.const.ts` `presentationGroupId` — a `legacyTrxGroupId` **is** that head; a party ledger reports under the control head of the side D3 parented it to; anything else answers **`null`**, deliberately, so an unplaceable ledger fails a census rather than landing its money in a row nobody posted to. `presentationJoinSql`/`presentationGroupExpr` are the same rule in SQL, written once for the ten statements that paste it. ⚠️ Transitional — it exists for the one release in which the ledger is the mechanism and the legacy chart is still the presentation, and goes with P3 and D9 |
| Why does the Day Book still print "Customer Dues" rather than the party? | it is a **label**, left on the shadow on purpose (P2b‑2): moving it now would relabel 5,393 lines to the *wrong* head's name on the way to the right one, and P3 turns it into the **ledger's** name, which is what Tally shows. Same for the cash book's `particulars` |
